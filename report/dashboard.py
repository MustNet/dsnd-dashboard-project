...
def align_features(X, model):
    ...
    model: fitted sklearn-like estimator with attrs:
           - feature_names_in_ (preferred) OR
           - n_features_in_
    Returns: numpy array aligned to model expectation (suitable for predict_proba)
    """
    import numpy as np
    import pandas as pd

    # if not DataFrame try to coerce
    ...
    # lowercase map for X columns
    cols = list(X.columns)
    lower_map = {c.lower(): c for c in cols}

    # If model provides feature names, create DataFrame in that order
    fnames = getattr(model, "feature_names_in_", None)
    if fnames is not None:
        target_cols = list(fnames)
        out = pd.DataFrame(index=[0], columns=target_cols, dtype=float)
        for tc in target_cols:
            lc = tc.lower()
            # direct match
            if lc in lower_map:
                out[tc] = X[lower_map[lc]].astype(float).values
            else:
                # try fuzzy match: map 'positive' -> 'positive_events' etc.
                if "positive" in lc:
                    candidate = next((c for c in cols if "positive" in c.lower()), None)
                    if candidate:
                        out[tc] = X[candidate].astype(float).values
                        continue
                if "negative" in lc:
                    candidate = next((c for c in cols if "negative" in c.lower()), None)
                    if candidate:
                        out[tc] = X[candidate].astype(float).values
                        continue
                # missing -> zero
    ...
    # fallback to n_features_in_ padding/truncation
    n = getattr(model, "n_features_in_", None)
    ...
    if vals.shape[1] == n:
        return vals
    if vals.shape[1] < n:
        pad = np.zeros((vals.shape[0], n - vals.shape[1]))
        return np.hstack([vals, pad])
    return vals[:, :n]
...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, entity_id, model):
        ...
        # align features to what the model expects, then predict
        try:
            X_aligned = align_features(X, self.predictor)
        except Exception:
            X_aligned = X

        try:
            proba = self.predictor.predict_proba(X_aligned)
            ...
            logging.getLogger(__name__).exception("Predict failed: %s", e)
            fig, ax = plt.subplots()
